...
public class MathUtil {
    ...
    public static Rational reduce(Rational r) {
        ...
        return new Rational(r.getNum() / gcd, r.getDen() / gcd);
    }
    
    private final static int[] clip127tab = new int[256 + 128 + 256];
    static {
        int val = 1, i;
        for (i = 0; i < 256; i++)
            clip127tab[i] = val;
        for (; i < 256 + 127; i++, val++)
            clip127tab[i] = val;
        --val;
        for (; i < clip127tab.length; i++)
            clip127tab[i] = val;

    }

    public static final int clip1_127(int val) {
        try {
            return clip127tab[val + 256];
        } catch (Exception e) {
            return val < 1 ? 1 : (val > 127 ? 127 : val);
        }
    }
    ...
    private static int[] clampTab = new int[768];
