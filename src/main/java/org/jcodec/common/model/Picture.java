...
public class Picture {
    ...
    public static Picture create(int width, int height, ColorSpace colorSpace) {
        return createCropped(width, height, 0, colorSpace, null);
    }

    public static Picture createCropped(int width, int height, int lowBitsNum, ColorSpace colorSpace, Rect crop) {
        ...
        byte[][] lowBits = null;
        if (lowBitsNum > 0) {
            lowBits = new byte[nPlanes][];
            for (int i = 0, plane = 0; i < MAX_PLANES; i++) {
                if (planeSizes[i] != 0) {
                    lowBits[plane++] = new byte[planeSizes[i]];
                }
            }
        }

        return new Picture(width, height, data, lowBits, colorSpace, lowBitsNum, crop);
    }
    ...
    public static Picture fromPictureHiBD(PictureHiBD pic) {
        ...
        Picture result = Picture.createCropped(pic.getWidth(), pic.getHeight(), lowBitsNum, pic.getColor(),
                pic.getCrop());
