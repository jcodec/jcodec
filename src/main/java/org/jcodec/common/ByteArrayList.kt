 ...
 */
class ByteArrayList(growAmount: Int) {
    ...
    fun toArray(): ByteArray {
        val result = ByteArray(_size + chunks.size * growAmount - _start)
        var off = 0
    ...
    fun fill(start: Int, end: Int, `val`: Byte) {
        var start = start
        var end = end
        start += _start
        end += _start
    ...
    operator fun contains(needle: Byte): Boolean {
    ...
    companion object {
        private const val DEFAULT_GROW_AMOUNT = 2048
        @JvmStatic
        fun createByteArrayList(): ByteArrayList {
            return ByteArrayList(DEFAULT_GROW_AMOUNT)
        }
    }
