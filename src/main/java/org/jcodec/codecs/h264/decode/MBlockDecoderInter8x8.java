...
public class MBlockDecoderInter8x8 extends MBlockDecoderBase {
    ...
    public void decode(int mb_type, Frame[][] references, Picture8Bit mb, SliceType sliceType, int mbIdx,
        ...
        if (sliceType == SliceType.P) {
            noSubMBLessThen8x8 = predict8x8P(references[0], mb, ref0, mbX, mbY, leftAvailable, topAvailable,
                    topLeftAvailable, topRightAvailable, x, pp);
            curMBType = P_8x8;
        } else {
            noSubMBLessThen8x8 = predict8x8B(references, mb, ref0, mbX, mbY, leftAvailable, topAvailable,
                    topLeftAvailable, topRightAvailable, x, pp);
            curMBType = B_8x8;
        }
        ...
        int codedBlockPattern = parser.readCodedBlockPatternInter(leftAvailable, topAvailable, s.leftCBPLuma
        ...
        if (s.transform8x8 && cbpLuma != 0 && noSubMBLessThen8x8) {
            transform8x8Used = parser.readTransform8x8Flag(leftAvailable, topAvailable, s.leftMBType, s.topMBType[mbX],
        ...
        decodeChromaResidual(leftAvailable, topAvailable, mbX, mbY, codedBlockPattern >> 4, qp1, qp2, MBType.P_16x16,
    ...
    private boolean predict8x8P(Picture8Bit[] references, Picture8Bit mb, boolean ref0, int mbX, int mbY,
            boolean leftAvailable, boolean topAvailable, boolean tlAvailable, boolean topRightAvailable, int[][][] x,
            PartPred[] pp) {
        int[] subMbTypes = new int[4];
        for (int i = 0; i < 4; i++) {
            subMbTypes[i] = parser.readSubMBTypeP();
        }
        Arrays.fill(pp, L0);
        int blk8x8X = mbX << 1;
        ...
        if (s.numRef[0] > 1 && !ref0) {
            refIdx[0] = parser.readRefIdx(leftAvailable, topAvailable, s.leftMBType, s.topMBType[mbX], L0, L0, L0, mbX,
                    0, 0, 2, 2, 0);
            refIdx[1] = parser.readRefIdx(true, topAvailable, P_8x8, s.topMBType[mbX], L0, L0, L0, mbX, 2, 0, 2, 2, 0);
            refIdx[2] = parser.readRefIdx(leftAvailable, true, s.leftMBType, P_8x8, L0, L0, L0, mbX, 0, 2, 2, 2, 0);
            refIdx[3] = parser.readRefIdx(true, true, P_8x8, P_8x8, L0, L0, L0, mbX, 2, 2, 2, 2, 0);
        }

        decodeSubMb8x8(subMbTypes[0], references, mbX << 6, mbY << 6, x[0], s.mvTopLeft[0], s.mvTop[0][mbX << 2],
                s.mvTop[0][(mbX << 2) + 1], s.mvTop[0][(mbX << 2) + 2], s.mvLeft[0][0], s.mvLeft[0][1], tlAvailable,
                topAvailable, topAvailable, leftAvailable, x[0][0], x[0][1], x[0][4], x[0][5], refIdx[0], mb, 0, 0, 0,
                mbX, s.leftMBType, s.topMBType[mbX], P_8x8, L0, L0, L0, 0);

        decodeSubMb8x8(subMbTypes[1], references, (mbX << 6) + 32, mbY << 6, x[0], s.mvTop[0][(mbX << 2) + 1],
                s.mvTop[0][(mbX << 2) + 2], s.mvTop[0][(mbX << 2) + 3], s.mvTop[0][(mbX << 2) + 4], x[0][1], x[0][5],
                topAvailable, topAvailable, topRightAvailable, true, x[0][2], x[0][3], x[0][6], x[0][7], refIdx[1], mb,
                8, 2, 0, mbX, P_8x8, s.topMBType[mbX], P_8x8, L0, L0, L0, 0);

        decodeSubMb8x8(subMbTypes[2], references, mbX << 6, (mbY << 6) + 32, x[0], s.mvLeft[0][1], x[0][4], x[0][5],
                x[0][6], s.mvLeft[0][2], s.mvLeft[0][3], leftAvailable, true, true, leftAvailable, x[0][8], x[0][9],
                x[0][12], x[0][13], refIdx[2], mb, 128, 0, 2, mbX, s.leftMBType, P_8x8, P_8x8, L0, L0, L0, 0);

        decodeSubMb8x8(subMbTypes[3], references, (mbX << 6) + 32, (mbY << 6) + 32, x[0], x[0][5], x[0][6], x[0][7],
                null, x[0][9], x[0][13], true, true, false, true, x[0][10], x[0][11], x[0][14], x[0][15], refIdx[3],
                mb, 136, 2, 2, mbX, P_8x8, P_8x8, P_8x8, L0, L0, L0, 0);
    ...
    private boolean predict8x8B(Frame[][] refs, Picture8Bit mb, boolean ref0, int mbX, int mbY, boolean leftAvailable,
            boolean topAvailable, boolean tlAvailable, boolean topRightAvailable, int[][][] x, PartPred[] p) {

        int[] subMbTypes = new int[4];
        for (int i = 0; i < 4; i++) {
            subMbTypes[i] = parser.readSubMBTypeB();
        ...
        for (int list = 0; list < 2; list++) {
            ...
            if (p[0].usesList(list))
                refIdx[list][0] = parser.readRefIdx(leftAvailable, topAvailable, s.leftMBType, s.topMBType[mbX],
                        s.predModeLeft[0], s.predModeTop[mbX << 1], p[0], mbX, 0, 0, 2, 2, list);
            if (p[1].usesList(list))
                refIdx[list][1] = parser.readRefIdx(true, topAvailable, B_8x8, s.topMBType[mbX], p[0],
                        s.predModeTop[(mbX << 1) + 1], p[1], mbX, 2, 0, 2, 2, list);
            if (p[2].usesList(list))
                refIdx[list][2] = parser.readRefIdx(leftAvailable, true, s.leftMBType, B_8x8, s.predModeLeft[1], p[0],
                        p[2], mbX, 0, 2, 2, 2, list);
            if (p[3].usesList(list))
                refIdx[list][3] = parser.readRefIdx(true, true, B_8x8, B_8x8, p[2], p[1], p[3], mbX, 2, 2, 2, 2, list);
        }
        ...
        for (int list = 0; list < 2; list++) {
            if (p[0].usesList(list))
                decodeSubMb8x8(bSubMbTypes[subMbTypes[0]], refs[list], mbX << 6, mbY << 6, x[list],
                        ...
                        mbb[list], 0, 0, 0, mbX, s.leftMBType, s.topMBType[mbX], B_8x8, s.predModeLeft[0],
                        s.predModeTop[blk8x8X], p[0], list);

            if (p[1].usesList(list))
                decodeSubMb8x8(bSubMbTypes[subMbTypes[1]], refs[list], (mbX << 6) + 32, mbY << 6, x[list],
                        s.mvTop[list][(mbX << 2) + 1], s.mvTop[list][(mbX << 2) + 2], s.mvTop[list][(mbX << 2) + 3],
                        s.mvTop[list][(mbX << 2) + 4], x[list][1], x[list][5], topAvailable, topAvailable,
                        topRightAvailable, true, x[list][2], x[list][3], x[list][6], x[list][7], refIdx[list][1],
                        mbb[list], 8, 2, 0, mbX, B_8x8, s.topMBType[mbX], B_8x8, p[0], s.predModeTop[blk8x8X + 1],
                        p[1], list);

            if (p[2].usesList(list))
                decodeSubMb8x8(bSubMbTypes[subMbTypes[2]], refs[list], mbX << 6, (mbY << 6) + 32, x[list],
                        s.mvLeft[list][1], x[list][4], x[list][5], x[list][6], s.mvLeft[list][2], s.mvLeft[list][3],
                        leftAvailable, true, true, leftAvailable, x[list][8], x[list][9], x[list][12], x[list][13],
                        refIdx[list][2], mbb[list], 128, 0, 2, mbX, s.leftMBType, B_8x8, B_8x8, s.predModeLeft[1],
                        p[0], p[2], list);

            if (p[3].usesList(list))
                decodeSubMb8x8(bSubMbTypes[subMbTypes[3]], refs[list], (mbX << 6) + 32, (mbY << 6) + 32,
    ...
    private void decodeSubMb8x8(int subMbType, Picture8Bit[] references, int offX, int offY,
        ...
        switch (subMbType) {
        case 3:
            decodeSub4x4(references, offX, offY, tl, t0, t1, tr, l0, l1, tlAvb, tAvb, trAvb, lAvb, x00, x01,
                    x10, x11, refIdx, mb, off, blk8x8X, blk8x8Y, mbX, leftMBType, topMBType, curMBType, leftPred,
                    topPred, partPred, list);
            break;
        case 2:
            decodeSub4x8(references, offX, offY, tl, t0, t1, tr, l0, tlAvb, tAvb, trAvb, lAvb, x00, x01, x10,
                    x11, refIdx, mb, off, blk8x8X, blk8x8Y, mbX, leftMBType, topMBType, curMBType, leftPred, topPred,
                    partPred, list);
            break;
        case 1:
            decodeSub8x4(references, offX, offY, tl, t0, tr, l0, l1, tlAvb, tAvb, trAvb, lAvb, x00, x01, x10,
                    x11, refIdx, mb, off, blk8x8X, blk8x8Y, mbX, leftMBType, topMBType, curMBType, leftPred, topPred,
                    partPred, list);
            break;
        case 0:
            decodeSub8x8(references, offX, offY, tl, t0, tr, l0, tlAvb, tAvb, trAvb, lAvb, x00, x01, x10, x11,
                    refIdx, mb, off, blk8x8X, blk8x8Y, mbX, leftMBType, topMBType, curMBType, leftPred, topPred,
                    partPred, list);
        }
    }

    private void decodeSub8x8(Picture8Bit[] references, int offX, int offY, int[] tl, int[] t0,
        ...
        int mvdX = parser.readMVD(0, lAvb, tAvb, leftMBType, topMBType, leftPred, topPred, partPred, mbX,
                blk8x8X, blk8x8Y, 2, 2, list);
        int mvdY = parser.readMVD(1, lAvb, tAvb, leftMBType, topMBType, leftPred, topPred, partPred, mbX,
                blk8x8X, blk8x8Y, 2, 2, list);
    ...
    private void decodeSub8x4(Picture8Bit[] references, int offX, int offY, int[] tl, int[] t0,
        ...
        int mvdX1 = parser.readMVD(0, lAvb, tAvb, leftMBType, topMBType, leftPred, topPred, partPred, mbX,
                blk8x8X, blk8x8Y, 2, 1, list);
        int mvdY1 = parser.readMVD(1, lAvb, tAvb, leftMBType, topMBType, leftPred, topPred, partPred, mbX,
                blk8x8X, blk8x8Y, 2, 1, list);
        ...
        int mvdX2 = parser.readMVD(0, lAvb, true, leftMBType, curMBType, leftPred, partPred, partPred, mbX,
                blk8x8X, blk8x8Y + 1, 2, 1, list);
        int mvdY2 = parser.readMVD(1, lAvb, true, leftMBType, curMBType, leftPred, partPred, partPred, mbX,
                blk8x8X, blk8x8Y + 1, 2, 1, list);
    ...
    private void decodeSub4x8(Picture8Bit[] references, int offX, int offY, int[] tl, int[] t0,
        ...
        int mvdX2 = parser.readMVD(0, true, tAvb, curMBType, topMBType, partPred, topPred, partPred, mbX,
                blk8x8X + 1, blk8x8Y, 1, 2, list);
        int mvdY2 = parser.readMVD(1, true, tAvb, curMBType, topMBType, partPred, topPred, partPred, mbX,
                blk8x8X + 1, blk8x8Y, 1, 2, list);
    ...
    private void decodeSub4x4(Picture8Bit[] references, int offX, int offY, int[] tl, int[] t0,
        ...
        int mvdX3 = parser.readMVD(0, lAvb, true, leftMBType, curMBType, leftPred, partPred, partPred, mbX,
                blk8x8X, blk8x8Y + 1, 1, 1, list);
        int mvdY3 = parser.readMVD(1, lAvb, true, leftMBType, curMBType, leftPred, partPred, partPred, mbX,
                blk8x8X, blk8x8Y + 1, 1, 1, list);
        ...
        int mvdX4 = parser.readMVD(0, true, true, curMBType, curMBType, partPred, partPred, partPred, mbX,
                blk8x8X + 1, blk8x8Y + 1, 1, 1, list);
        int mvdY4 = parser.readMVD(1, true, true, curMBType, curMBType, partPred, partPred, partPred, mbX,
                blk8x8X + 1, blk8x8Y + 1, 1, 1, list);
