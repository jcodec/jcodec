...
public class Intra4x4PredictionBuilder {
    ...
    public static void predictWithMode(int mode, int[] residual, boolean leftAvailable, boolean topAvailable,
            boolean topRightAvailable, byte[] leftRow, byte[] topLine, byte topLeft[], int mbOffX, int blkX, int blkY, byte[] pixOut) {
        switch (mode) {
        case 0:
    ...
    public static void predictHorizontal(int[] residual, boolean leftAvailable, byte[] leftRow,
    ...
    public static void predictDiagonalDownLeft(int[] residual, boolean topAvailable,
            boolean topRightAvailable, byte[] topLine, int mbOffX, int blkX, int blkY, byte[] pixOut) {
        ...
        for (int y = 0; y < 4; y++) {
            for (int x = 0; x < 4; x++) {
                if (x > y) {
                    ...
                    pixOut[off + x] = (byte)clip(residual[(y << 2) + x] + ((t1 + 2 * t2 + t3 + 2) >> 2), -128, 127);
                } else if (x < y) {
                    int l1;
                    if (y - x - 2 == -1)
                        l1 = topLeft[blkY >> 2];
                    ...
                    pixOut[off + x] = (byte)clip(residual[(y << 2) + x] + ((l1 + 2 * l2 + l3 + 2) >> 2), -128, 127);
                } else
                    pixOut[off + x] = (byte)clip(residual[(y << 2) + x]
                            + ((topLine[mbOffX + blkX + 0] + 2 * topLeft[blkY>>2] + leftRow[blkY] + 2) >> 2), -128, 127);
            }
            off += 16;
        }
    }
    ...
    public static void predictHorizontalUp(int[] residual, boolean leftAvailable, byte[] leftRow,
            int mbOffX, int blkX, int blkY, byte[] pixOut) {
