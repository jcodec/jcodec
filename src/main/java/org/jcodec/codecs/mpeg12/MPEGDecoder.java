...
public class MPEGDecoder extends VideoDecoder {
	...
	public static DemuxerTrackMeta getMeta(ByteBuffer data) {
		ByteBuffer codecPrivate = getSequenceHeader(data.duplicate());
		SequenceHeader sh = SequenceHeader.read(codecPrivate.duplicate());
		return new DemuxerTrackMeta(TrackType.VIDEO, Codec.MPEG2, null, 0, 0,
				new Size(sh.horizontal_size, sh.vertical_size), codecPrivate);
	}
