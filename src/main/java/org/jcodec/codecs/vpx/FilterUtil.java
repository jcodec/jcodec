...
public class FilterUtil {
     ...
     */
    private static short clipPlus128(short v) {
        return (short) (CommonUtils.byteClamp(v) + 128);
    }
    ...
    public static class Segment {
        ...
        public boolean isFilterRequired(int interior, int edge) {
            return ((abs(p0 - q0) << 2) + (abs(p1 - q1) >> 2)) <= edge && abs(p3 - p2) <= interior
                    && abs(p2 - p1) <= interior && abs(p1 - p0) <= interior && abs(q3 - q2) <= interior
                    && abs(q2 - q1) <= interior && abs(q1 - q0) <= interior;
        }

        /**
         * HEV - Hight Edge Variance. Filtering is altered if (at least) one of the
         * differences on either side of the edge exceeds a threshold (we have "high
         * edge variance").
         * 
         * @param threshold
         * @param p1        before
        ...
        void filterMb(int hevThreshold, int interiorLimit, int edgeLimit) {
            ...
            if (signedSeg.isFilterRequired(interiorLimit, edgeLimit)) {
                if (!signedSeg.isHighVariance(hevThreshold)) {
                    ...
                    int w = CommonUtils.byteClamp(
                            (short) (CommonUtils.byteClamp((short) (signedSeg.p1 - signedSeg.q1))
                                    + 3 * (signedSeg.q0 - signedSeg.p0)));
                    ...
                    short a = (short) ((27 * w + 63) >> 7);

                    q0 = clipPlus128((short) (signedSeg.q0 - a));
                    p0 = clipPlus128((short) (signedSeg.p0 + a));
                    // Next two are adjusted by 2/7 the edge difference
                    a = (short) ((18 * w + 63) >> 7);
                    // System.out.println("a: "+a);
                    q1 = clipPlus128((short) (signedSeg.q1 - a));
                    p1 = clipPlus128((short) (signedSeg.p1 + a));
                    // Last two are adjusted by 1/7 the edge difference
                    a = (short) ((9 * w + 63) >> 7);
                    q2 = clipPlus128((short) (signedSeg.q2 - a));
                    p2 = clipPlus128((short) (signedSeg.p2 + a));
                } else
        ...
        public void filterSb(int hev_threshold, int interior_limit, int edge_limit) {
            ...
            if (signedSeg.isFilterRequired(interior_limit, edge_limit)) {
                ...
                short a = (short) ((this.adjust(hv) + 1) >> 1);
                if (!hv) {
                    this.q1 = clipPlus128((short) (signedSeg.q1 - a));
                    this.p1 = clipPlus128((short) (signedSeg.p1 + a));
                }
            }
        }
        ...
        private int adjust(boolean use_outer_taps) {
             ...
             */
            short a = CommonUtils.byteClamp(
                    (short) ((use_outer_taps ? CommonUtils.byteClamp((short) (p1 - q1)) : 0) + 3 * (q0 - p0)));

            /**
             * b is used to balance the rounding of a/8 in the case where the "fractional"
             * part "f" of a/8 is exactly 1/2.
             */
            short b = (short) ((CommonUtils.byteClamp((short) (a + 3)) >> 3));
             ...
             */
            a = (short) (CommonUtils.byteClamp((short) (a + 4)) >> 3);

            /**
             * Subtract "a" from q0, "bringing it closer" to p0.
             */
            this.q0 = clipPlus128((short)(q0 - a));

            /**
             * Add "a" (with adjustment "b") to p0, "bringing it closer" to q0. T he clamp
             * of "a+b", while present in the reference decoder, is superfluous; we have -16
             * <= a <= 15 at this point.
             */
            this.p0 = clipPlus128((short)(p0 + b));
    ...
    private static short minus128(short v) {
        return (short) (v - 128);
    }
