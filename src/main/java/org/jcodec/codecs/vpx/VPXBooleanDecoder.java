...
//                System.out.println("read value: " + value + " binary: " + Integer.toBinaryString(value));
     ...
     * <ul>
     * <li>if tree element is a positive number it is treated as index of the child
     * elements
     * 
     * <pre>
     * tree[i] > 0
     * </pre>
     * <ul>
     * <li>left child is assumed to have index
     * 
     * <pre>
     * i
     * </pre>
     ...
     * 
     * </li>
     * <li>right child is assumed to have index
     * 
     * <pre>
     * i + 1
     * </pre>
     ...
     * </ul>
     * </li>
     * <li>a negative tree value means a leaf node was reached and it's negated
     ...
     * <pre>
     * 
     * On the other hand probabilities are coded only for non-leaf nodes. Thus tree
     * array has twice as many nodes as probabilities array Consider (3>>1) == 1 ==
     * (2>>1), and (0>>1) == 0 == (1>>1) Thus single probability element refers to
     * single parent element in tree. if (using that probability) a '0' is coded,
     * algorithm goes to the left branch, correspondingly if '1' is coded, algorithm
     * goes to the right branch (see tree structure above).
     * 
     * The process is repeated until a negative tree element is found.
     * 
     */
    public short readTree(short tree[], short probability[]) {
        short i = 0;

        /*
         * 1. pick corresponding probability probability[i >> 1] 2. pick left or right
         * branch from coded info decodeBool(probability) 3. tree[i+decodedBool] get
         * corresponding (left of right) value 4. repeat until tree[i+decodedBool] is
         * positive
         */
        while ((i = tree[i + readBit(probability[i >> 1])]) > 0) {
        }
        return (short) -i; /* negate the return value */

    }

    // There is a method in the class (or one of its parents) having the same name
    // with the method named [readTree] but is less generic
    ...
    public short readTreeSkip(short t[], /* tree specification */
            short p[], /* corresponding interior node probabilities */
            short skip_branches) {
        short i = (short) (skip_branches * 2); /* begin at root */

        /* Descend tree until leaf is reached */
        while ((i = t[i + readBit(p[i >> 1])]) > 0) {
        }
        return (short) -i; /* return value is negation of nonpositive index */
    ...
    public static int leadingZeroCountInByte(byte b) {
        ...
        /*
         * if-less alternative: http://aggregate.ee.engr.uky.edu/MAGIC/#Leading Zero
         * Count
         */
    }
}