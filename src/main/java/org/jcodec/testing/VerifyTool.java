...
public class VerifyTool {
    ...
    private boolean test(File coded, File ref) throws IOException {
        ...
        while ((nextFrame = es.nextFrame()) != null) {
            ...
            if (!Platform.arrayEqualsInt(JCodecUtil2.getAsIntArray(yuv, lumaSize), pic.getPlaneData(0)))
                return false;
            if (!Platform.arrayEqualsInt(JCodecUtil2.getAsIntArray(yuv, crSize), pic.getPlaneData(1)))
                return false;
            if (!Platform.arrayEqualsInt(JCodecUtil2.getAsIntArray(yuv, cbSize), pic.getPlaneData(2)))
                return false;
        }
        return true;
    }
